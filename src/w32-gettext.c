/* w32-gettext.c - A simple gettext implementation for Windows targets.
...
/* Convert UTF8 to the native codepage.  This function is guaranteed
   to never return NULL.  Caller must free the return value.  */
char *
utf8_to_native (const char *string)
{
  ...
  if (!wstring)
    return strdup ("[Error: utf8_to_wchar failed]");

  result = wchar_to_native (wstring);
  free (wstring);
  if (!result)
    result = strdup ("[Error: wchar_to_native failed]");
...
/* Convert native character set to utf-8.  This is required if we want
   ...
   never returned.  Caller must free the return value.  */
char *
native_to_utf8 (const char *string)
{
  ...
  if (!wstring)
    return strdup ("[Error: native_to_wchar failed]");

  result = wchar_to_utf8 (wstring);
  free (wstring);
  if (!result)
    result = strdup ("[Error: wchar_to_utf8 failed]");
